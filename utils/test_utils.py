...
def compute_metrics(model_name, bevel, begen, begt, dataset, test = False, data = False):
    ...
    for i in range(mask.shape[0]):
        ...
        if count_ == 0:
            ...
            temp_psnr = 10* np.log( M**2 / mse_list[-1])
            if str(temp_psnr) not in ["nan", "inf"]:
    ...
    return MAE, MAE_sd, MSE, MSE_sd, PSNR, PSNR_sd, mae_list, mse_list, psnr_list

# Compute SSIMs
def compute_ssim(model, dataset, acm = False, sqrt = False, test = False, Y_pred = None, pct = False, data = False):
    ...
    if not acm:
        ...
        #ssim_sum = np.zeros((3))
        ssim_sum = np.zeros((4))
        ssims_0, ssims_1, ssims_2, ssims_3, = [], [], [], []
        ssims = [ssims_0, ssims_1, ssims_2, ssims_3]
        ...
        sd_2 = np.std(ssims[2])
        sd_3 = np.std(ssims[3])
        sd = [sd_0, sd_1, sd_2, sd_3]

    return ssim_sum, sd, ssims

def compute_val_metrics(models, model_names, datasets, bevels, begens, begts, sqrt = False, acm = False, test = False, pct = False):
    MAE, MAE_sd, MSE, MSE_sd, PSNR, PSNR_sd, SSIM, SSIM_sd, mae_L, mse_L, psnr_L, ssim_L = [], [], [], [], [], [], [], [], [], [], [], []
    for i in tqdm(range(len(model_names))):
        if type(datasets) == list:
            ...
            dataset = datasets
        if pct:
            mae, mae_sd, mse, mse_sd, psnr, psnr_sd, mae_list, mse_list, psnr_list = compute_metrics(model_names, bevels, begens, begts, dataset, data = True)
        else:
            mae, mae_sd, mse, mse_sd, psnr, psnr_sd, mae_list, mse_list, psnr_list = compute_metrics(model_names[i], bevels[i], begens[i], begts[i], dataset, data = True)
        mae_L.append(mae_list)
        mse_L.append(mse_list)
        psnr_L.append(psnr_list)
        #ssim_L.append(ssim_list)
        MAE.append(mae)
        ...
        if not acm:
            ssim, ssim_sd, ssim_list = compute_ssim(models[i], dataset, sqrt = sqrt, test = test, pct = pct, data = True)
            SSIM.append(ssim)
            SSIM_sd.append(ssim_sd)
            ssim_L.append(ssim_list)
    if acm:
        SSIM, SSIM_sd, ssim_L = compute_ssim(models, datasets, sqrt = sqrt, acm = True, data = True)
        ...
        df = pd.DataFrame({' ': model_names, 'MAE': MAE, '$\sigma_{MAE}$': MAE_sd, 'MSE': MSE, '$\sigma_{MSE}$': MSE_sd, 'PSNR': PSNR, '$\sigma_{PSNR}$': PSNR_sd, 'SSIM': SSIM, '$\sigma_{SSIM}$': SSIM_sd}).set_index(' ')
        pd.set_option('display.float_format', '{:.4f}'.format)
    return df, mae_L, mse_L, psnr_L, ssim_L
...
def compute_test_metrics(model, model_name, dataset, recon_3d = False, width_2d = 256, sqrt = False, pct = False):
    ...
    if pct:
        ...
        mae, mae_sd, mse, mse_sd, psnr, psnr_sd, mae_list, mse_list, psnr_list = compute_metrics(model_name, bevel, begen, begt, dataset, test = True)
        ssim, ssim_sd, ssim_list = compute_ssim(model, dataset, sqrt = sqrt, test = True, Y_pred = gen_vol)
       ...
       # mae, mae_sd, mse, mse_sd, psnr, psnr_sd, mae_list, mse_list, psnr_list = compute_metrics(model_name, bevel, begen, begt, dataset, test = True)
        tuple_test = compute_metrics(model_name, bevel, begen, begt, dataset, test= True)
        mae, mae_sd, mse, mse_sd, psnr, psnr_sd, mae_list, mse_list, psnr_list = tuple_test
        print("Length of tuple: ", len(tuple_test))
        ssim, ssim_sd, ssim_list = compute_ssim(model, dataset, sqrt = sqrt, test = True, Y_pred = gen_vol)
    
    else:
        S_slices = np.zeros((1,256,256,256,1))
        C_slices = np.zeros((1,256,256,256,1))
        T_slices = np.zeros((1,256,256,256,1))
        nb_sums = np.zeros((1,256,256,256,1))
        ...
        for i in tqdm(range(256)): # volume is 256x256x256
            for j in range(N_crops_per_dir):
                for k in range(N_crops_per_dir):
                    S_slices[:, i, j*s:j*s+l, k*s:k*s+l, :] += model.predict( X[:, i, j*s:j*s+l, k*s:k*s+l, :] )
                    C_slices[:, i, j*s:j*s+l, k*s:k*s+l, :] += model.predict( X[:, j*s:j*s+l, i, k*s:k*s+l, :] )
                    T_slices[:, i, j*s:j*s+l, k*s:k*s+l, :] += model.predict( X[:, j*s:j*s+l, k*s:k*s+l, i, :] )
    ...
    return df, gen_vol, mae_list, mse_list, psnr_list, ssim_list #, MAE_slices
    #return df, gen_vol, tuple_test(6), tuple_test(7), tuple_test(8), ssim_list
